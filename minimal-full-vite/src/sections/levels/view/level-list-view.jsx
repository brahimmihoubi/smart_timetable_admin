...
import { LevelTableToolbar } from '../level-table-toolbar';
...
const LEVEL_NAMES = ['L1', 'L2', 'L3', 'M1', 'M2'];
const DEPARTMENTS = ['Computer Science', 'Mathematics', 'Physics'];

const INITIAL_LEVELS = [
  { id: '1', name: 'L1', department: 'Computer Science' },
  { id: '2', name: 'L2', department: 'Computer Science' },
  { id: '3', name: 'M1', department: 'Mathematics' },
];
...
export function LevelListView() {
  ...
  const [tableData, setTableData] = useState(INITIAL_LEVELS);
  ...
  const handleOpenEdit = useCallback(
    (row) => {
      setEditRow(row);
      setFormData({ name: row.name, department: row.department });
      openForm.onTrue();
    },
    [openForm]
  );
  ...
  return (
    <>
      <DashboardContent>
        ...
        <Card>
          <LevelTableToolbar filters={filters} onResetPage={table.onResetPage} />
      ...
      <Dialog open={openForm.value} onClose={openForm.onFalse} fullWidth maxWidth="sm">
        <DialogTitle>{editRow ? 'Edit Level' : 'New Level'}</DialogTitle>
        <DialogContent>
